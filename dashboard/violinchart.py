...
def _prepare_event_data(filtered_data):
    """
    Prepare event data.
    Now treats 'None' identically to other events but keeps it at the end of the axis.

    Returns:
        Tuple of (processed_data, unique_events, event_map).
    """
    ...
    # We filter out "None" first to sort the active events alphabetically
    unique_events = sorted([e for e in filtered_data["event_display"].unique() if e.lower() != "none"])

    # If "None" exists in the data, append it to the end so it appears last on the X-axis
    if filtered_data["event_display"].str.lower().eq("none").any():
        unique_events.append("None")

    # Map event names to integer positions
    event_map = {evt: i for i, evt in enumerate(unique_events)}

    return filtered_data, unique_events, event_map
...
def _add_violin_traces(
    ...
    for service in selected_services:
        ...
        # This works for 'None' too since it's now in the event_map
